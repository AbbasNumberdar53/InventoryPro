...
const BuyProductForm = () => {
  ...
  //Fetching all modelnames /api/purchased/${brandname}?brand=${brandname}&query=modelname
  //   useEffect(() => {
  //     const fetchModelNames = async () => {
  //       try {
  //         setIsLoading(true);
  //         const response = await fetch(
  //           `/api/AddedProducts?brand=${brandname}&query=modelname`
  //         );
  //         if (response.ok) {
  ...
  //             setModelnames([]);
  //           } else {
  //             //Created a Set and assigning its values to an array to take only unique values
  //             var modelSet = new Set(data.output.modelname);
  //             var modelarr = [];
  //             modelSet.forEach((value) => modelarr.push(value));

  //             //Set Modelnames state
  //             setModelnames(modelarr);

  //             // Set default value of modelname to the first element of the modelname array of output object
  //             form.setValue("modelname", data.output?.modelname[0] ?? "");
